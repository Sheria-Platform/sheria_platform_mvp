...
class RAGIngestionPipeline:
    ...
    def process_document(self, bucket_name: str, object_name: str) -> Dict:
        ...
        """
        from pipelines.ingestion.loaders import pdf, docx, html
    ...
    def ingest_bucket(self, bucket_name: str, mode: str = "dev"):
        ...
        if success_count > 0:
            ...
            for r in results:
                if r["status"] == "success":
                    print(f"  ✓ {r['file']}: {r['chunks']} chunks, {r['embeddings']} embeddings, {r['entities']} entities")
...
def main():
    ...
    training_files = uploader.upload_directory(
    ...
    # Upload dev data (20%)
    print("\n>> Uploading DEV data...")
    dev_files = uploader.upload_directory(
        config.DEV_BUCKET, 
        split_ratio=0.8
    )
